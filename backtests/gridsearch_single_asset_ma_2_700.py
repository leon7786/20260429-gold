...
def main(asset: str):
    ...
    with ProcessPoolExecutor(max_workers=workers, initializer=init_worker, initargs=(close, prev_close, ma_matrix)) as ex:
        ...
        for f in as_completed(futs):
            ...
            if done % 100 == 0 or done == len(buy_list):
                print(f'progress: {done}/{len(buy_list)} buy_ma')
    ...
    out_summary = OUT_DIR / f'{asset}_best_ma_2_700_vs_2_700_summary.csv'
...
if __name__ == '__main__':
    if len(sys.argv) < 2 or sys.argv[1] not in ASSETS:
        print('usage: python gridsearch_single_asset_ma_2_700.py <asset>')
