...
PRICE_PATH = BASE / 'data' / 'brent.csv'
OUT_DIR = BASE / 'backtest_results'
OUT_DIR.mkdir(parents=True, exist_ok=True)

BUY_MIN, BUY_MAX = 1, 2000
SELL_MIN, SELL_MAX = 1, 2000
...
def main():
    ...
    raw['close'] = pd.to_numeric(raw['Close'] if 'Close' in raw.columns else raw['close'], errors='coerce')
    ...
    with ProcessPoolExecutor(max_workers=workers, initializer=init_worker, initargs=(close, prev_close, ma_matrix)) as ex:
        ...
        for f in as_completed(futs):
            ...
            if done % 20 == 0 or done == len(futs):
                print(f'progress: {done}/{len(buy_list)} buy_ma finished')
    ...
    res_path = OUT_DIR / 'oil_ma_gridsearch_1_2000_vs_1_2000.csv'
    res.to_csv(res_path, index=False)

    robust = res[res['trades'] >= 30].head(50)
    robust_path = OUT_DIR / 'oil_ma_gridsearch_1_2000_vs_1_2000_top50_trades_ge30.csv'
    ...
    cmp_path = OUT_DIR / 'oil_best_vs_buyhold_1_2000_vs_1_2000.csv'
    ...
    print('buy_hold_return_pct', bh)
    print('excess_vs_buy_hold_pct', float(best['total_return_pct'] - bh))
